...
fn main() {
    let manifest_dir = env::var_os("CARGO_MANIFEST_DIR").expect("Failed to find CARGO_MANIFEST_DIR");

    let mut libwebp_dir = PathBuf::from(manifest_dir);
    libwebp_dir.push("libwebp");

    let mut cc = cc::Build::new();
    cc.include(&libwebp_dir);
    cc.define("NODEBUG", Some("1"));
